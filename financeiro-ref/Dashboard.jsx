  ...
  return (
    <div style={{ animation: "fadeIn .3s" }}>
      ...
      <div className="mobile-only" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {items.map(function(it, i) {
          var isExpM = expanded[it.id];
          var hasSubsM = it.subs && it.subs.length > 0;
          return (
            <div key={it.id || i} style={{ background: C.card, border: "1px solid " + C.border, borderRadius: 12, overflow: "hidden", opacity: it.pago ? 0.5 : 1 }}>
              <div style={{ padding: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {canExpand && (hasSubsM || isExpM) ? <button onClick={function() { toggleExpand(it.id); }} style={{ width: 24, height: 24, borderRadius: 6, background: C.bB, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: C.b, cursor: "pointer", border: "none", flexShrink: 0, transition: "transform .2s", transform: isExpM ? "rotate(90deg)" : "none" }}>▶</button> : null}
                  <input type="checkbox" checked={!!it.pago} onChange={function() { upItem(activeTab, it.id, "pago", !it.pago); }} style={{ cursor: "pointer", width: 18, height: 18, accentColor: C.g, flexShrink: 0 }} />
                  <div style={{ flex: 1, minWidth: 0 }}><EText value={it.nome} onChange={function(v) { upItem(activeTab, it.id, "nome", v); }} placeholder="Nome..." w={140} /></div>
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 16, fontWeight: 700, color: clr, flexShrink: 0 }}>{fmt(it.valor)}</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8, paddingTop: 8, borderTop: "1px solid " + C.border }}>
                  {isCart && <span style={{ background: C.pB, color: C.p, padding: "3px 10px", borderRadius: 6, fontSize: 10, fontWeight: 600 }}>{it.cartao || ""}</span>}
                  <span style={{ background: C.bB, color: C.b, padding: "3px 8px", borderRadius: 5, fontSize: 10, fontWeight: 500 }}>{"Dia " + (it.dataPg || "—")}</span>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: priColors[it.prioridade || "normal"], flexShrink: 0 }}></span>
                  <div style={{ display: "flex", gap: 4, marginLeft: "auto" }}>
                    {canExpand ? <button onClick={function() { addSub(activeTab, it.id); }} style={{ background: C.card, border: "none", color: C.b, width: 28, height: 28, borderRadius: 6, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>+</button> : null}
                    <button onClick={function() { recur(activeTab, it); }} style={{ background: C.card, border: "none", color: C.dm, width: 28, height: 28, borderRadius: 6, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>⟳</button>
                    <button onClick={function() { if (window.confirm("Remover?")) rmItem(activeTab, it.id); }} style={{ background: C.card, border: "none", color: C.r, width: 28, height: 28, borderRadius: 6, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>✕</button>
                  </div>
                </div>
              </div>
              {canExpand && isExpM && (
                <div style={{ background: "rgba(59,130,246,.02)", borderTop: "1px solid " + C.border, padding: "8px 14px 8px 38px" }}>
                  {(it.subs || []).map(function(sub, si) {
                    return <div key={si} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid rgba(255,255,255,.03)", fontSize: 12 }}>
                      <span style={{ color: C.dm, display: "flex", alignItems: "center", gap: 4 }}><span style={{ color: C.dm2, fontSize: 10 }}>└</span> <EText value={sub.nome} onChange={function(v) { upSub(activeTab, it.id, si, "nome", v); }} placeholder="..." w={90} /></span>
                      <span style={{ display: "flex", alignItems: "center", gap: 6 }}><ENum value={sub.valor} onChange={function(v) { upSub(activeTab, it.id, si, "valor", v); }} color={C.dm} w={55} /><button onClick={function() { rmSub(activeTab, it.id, si); }} style={{ background: "none", border: "none", color: C.dm2, cursor: "pointer", fontSize: 11 }}>✕</button></span>
                    </div>;
                  })}
                  <button onClick={function() { addSub(activeTab, it.id); }} style={{ color: C.b, fontSize: 11, cursor: "pointer", padding: "4px 0", background: "none", border: "none" }}>+ detalhe</button>
                </div>
              )}
            </div>
          );
        })}
        <button onClick={function() { addItem(activeTab); }} style={{ width: "100%", padding: 14, background: "transparent", border: "1px dashed " + clr + "30", borderRadius: 12, color: clr, fontSize: 13, cursor: "pointer", fontWeight: 600 }}>+ {isCart ? "Novo cartão" : activeTab === "entradas" ? "Nova entrada" : "Nova despesa"}</button>
        {isCart ? <button onClick={onAddCartao} style={{ width: "100%", padding: 14, marginTop: 6, background: "transparent", border: "1px dashed " + C.c + "30", borderRadius: 12, color: C.c, fontSize: 13, cursor: "pointer", fontWeight: 600 }}>+ Novo Cartão</button> : null}
      </div>
