...
function TasksWidget() {
  ...
  return (
    <Widget icon="✅" title="Tarefas" to="/tarefas">
      {isLoading ? (
...
function HabitsWidget() {
  ...
  return (
    <Widget icon="🔥" title="Hábitos" to="/habitos">
      {isLoading ? (
...
function ProjectsWidget() {
  ...
  const STATUS_COLOR: Record<string, string> = {
    'em dev': '#60a5fa', início: '#fbbf24', ativo: '#34d399',
    live: '#6ee7b7', pausado: '#555',
  }

  return (
    <Widget icon="📁" title="Projetos" to="/projetos">
      {isLoading ? (
...
function GoalsWidget() {
  ...
  return (
    <Widget icon="🎯" title="Metas" to="/metas">
      {isLoading ? (
        <div className="space-y-2"><Sk w="w-14" h="h-6" /><Sk /><Sk w="w-4/5" /></div>
      ) : (
        <>
          <BigStat value={`${avgPct}%`} label="progresso médio" color={colorAvg} />
...
function FinanceWidget() {
  ...
  return (
    <Widget icon="💰" title="Financeiro" to="/financeiro">
      {isLoading ? (
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          {tile('Saídas',   data?.saidas   ?? 0, '#f87171')}
...
function InvoicesWidget() {
  ...
  return (
    <Widget icon="🧾" title="Faturamento" to="/faturamento">
      {isLoading ? (
        <div className="space-y-2"><Sk w="w-32" h="h-6" /><Sk w="w-24" /></div>
      ) : (
        <>
          <BigStat
            value={fmtBRL(data?.total ?? 0)}
            label="a receber"
            color="#0EA5E9"
          />
...
function SportsWidget() {
  ...
  return (
    <Widget icon="🏃" title="Esportes" to="/esportes">
      {isLoading ? (
...
function NotesWidget() {
  ...
  return (
    <Widget icon="📝" title="Notas" to="/notas">
      {isLoading ? (
...
function LibraryWidget() {
  ...
  return (
    <Widget icon="📚" title="Biblioteca" to="/biblioteca">
      {isLoading ? (
          ...
          {current ? (
            <div className="mt-3 bg-bg rounded-xl px-3 py-2.5">
              ...
              {current.total_pages && current.pages_read ? (
                <>
                  <Bar
                    pct={(current.pages_read / current.total_pages) * 100}
                    color="#0EA5E9"
                  />
...
function StreakWidget() {
  ...
  return (
    <Widget icon="⚡" title="Maior Streak" to="/habitos">
      {isLoading ? (
        ...
        <div>
          ...
          <p className="text-ink-3 text-xs mt-1">Marque hábitos para iniciar streak.</p>
        </div>
      ) : (
        <>
          ...
          <div className="text-ink-2 text-xs truncate">{topStreak.name}</div>
          <div className="mt-2">
    ...
    <div>
      ...
      <div className="mb-7">
        ...
        <p
          ...
          style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13 }}
        >
          {longDate()}
        </p>
      </div>
