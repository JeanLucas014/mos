  ...
  return (
    <div className="flex-1 overflow-y-auto h-full px-6 py-4 space-y-6">
      {sortedDates.map(dateKey => {
        ...
        return (
          <div key={dateKey}>
            <div className="text-xs text-ink-3 uppercase tracking-wider font-[Sora] mb-2 sticky top-0 bg-[#0a0a0a] py-1 capitalize">
              {d.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}
            </div>
            <div className="space-y-2">
              {grouped[dateKey].map(ev => (
                <div key={ev.id} onClick={() => onSelectEvent(ev)}
                  className="flex items-start gap-3 bg-[#111111] border border-[#1f1f1f] rounded-xl p-3 cursor-pointer hover:border-brand/30 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!ev.all_day && (
                      <div className="text-[11px] text-ink-3 mt-0.5 flex items-center gap-2">
                        ...
                        {ev.recurrence_rule && (
                          <span className="text-[10px] text-ink-3">↻ Recorrente</span>
                        )}
                      </div>
                    )}
                    {ev.location && (
                      <div className="text-[11px] text-ink-3 mt-0.5">📍 {ev.location}</div>
                    )}
                    {ev.description && (
