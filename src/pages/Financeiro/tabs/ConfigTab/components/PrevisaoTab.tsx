      ...
      <div className="bg-bg-2 border border-line rounded-xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleAdd}
            className="px-4 py-1.5 text-sm font-medium bg-brand text-black rounded-lg hover:bg-[#38bdf8]">
            <Plus size={14} />
          </button>
        </div>
      </div>
      ...
      {previsaoItems.map(item => (
        <div key={item.id} className="group flex items-center gap-3 py-2.5 border-b border-line">
          ...
          <input
            ...
            className="w-28 bg-bg border border-line rounded-lg px-3 py-1 text-xs text-white outline-none tabular-nums text-right focus:border-brand/60"
          />
          <button onClick={() => onDelete(item.id)}
            className="opacity-0 group-hover:opacity-100 text-ink-3 hover:text-[#ef4444] transition-all">
            <Trash2 size={12} />
          </button>
        </div>
      ))}
        ...
        return (
          <div className="bg-bg-2 border border-line rounded-xl p-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-ink-3">Total mensal</span>
              <span className="text-white font-medium tabular-nums">
                {total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-ink-3">Dividido por {daysThisMonth} dias</span>
              <span className="text-brand font-bold tabular-nums text-base">
